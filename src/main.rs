...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if args.len() != 3 {
        eprintln!("Usage: {} <socket-name> <backend-unix-socket-path>", args[0]);
        std::process::exit(1);
    }
...
async fn handle_connection(
    ...
    let (backend_copied, client_copied) = tokio::join!(
        copy_with_abort(&mut backend_read, &mut client_write, cancel.subscribe())
            .then(|r| { let _ = cancel.send(()); async { r } }),
        copy_with_abort(&mut client_read, &mut backend_write, cancel.subscribe())
            .then(|r| { let _ = cancel.send(()); async { r } })
    );
